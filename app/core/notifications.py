...
class Notifications:
    ...
    def __init__(self):
        ...
        try:
            ...
            if settings.FIREBASE_SERVICE_ACCOUNT_PATH:
                ...
                logger.warning(
                    "FIREBASE_SERVICE_ACCOUNT_PATH not configured, "
    ...
    def send_match_notification(
        ...
        try:
            ...
            logger.info(
                f"Match notification sent successfully to token: "
                f"{fcm_token[:10]}..."
            )
        except Exception as e:
            logger.error(
                f"Failed to send match notification: {str(e)}",
                exc_info=True
            )
    ...
    def send_chat_notification(
        ...
        if not partner_fcm_token:
            logger.warning(
                "Partner FCM token is missing, "
                "skipping chat notification"
            )
            return
        ...
        try:
            ...
            messaging.send(message)
            logger.info(
                f"Chat notification sent successfully to token: "
                f"{partner_fcm_token[:10]}..."
            )
        except Exception as e:
            logger.error(
                f"Failed to send chat notification: {str(e)}",
                exc_info=True
            )
    ...
    def send_payment_notification(
        ...
        try:
            ...
            messaging.send(message)
            logger.info(
                f"Payment notification sent successfully to token: "
                f"{fcm_token[:10]}..."
            )
        except Exception as e:
            logger.error(
