...
def get_access_token() -> Optional[str]:
    ...
    if not settings.CONSUMER_KEY or not settings.CONSUMER_SECRET:
        logger.error(
            "CONSUMER_KEY or CONSUMER_SECRET not set "
    ...
    if not settings.DARAJA_CREDENTIALS_URL:
        logger.error(
            "DARAJA_CREDENTIALS_URL not set "
            "in environment variables"
        )
        return None
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            logger.error(
                f"Failed to get Daraja access token: "
                f"{response.status_code} - {response.text}"
            )
            consumer_key_preview = (
                settings.CONSUMER_KEY[:10]
                if settings.CONSUMER_KEY else 'NOT SET'
            )
            logger.error(
                f"Consumer Key (first 10 chars): "
                f"{consumer_key_preview}..."
            )
            return None
    except Exception as e:
        logger.error(
            f"Error getting Daraja access token: {str(e)}",
            exc_info=True
        )
        return None
...
def initiate_stk_push(
    ...
    if not settings.SHORT_CODE or not settings.DARAJA_PASSKEY:
        logger.error(
            "SHORT_CODE or DARAJA_PASSKEY not set "
            "in environment variables"
        )
        return {
            "error": (
                "Daraja credentials not configured. "
                "Please set SHORT_CODE and DARAJA_PASSKEY"
            )
        }, 500

    if not settings.DARAJA_STK_PUSH_URL:
        logger.error(
            "DARAJA_STK_PUSH_URL not set "
            "in environment variables"
        )
    ...
    if access_token is None:
        return {
            "error": (
                ...
                "Please check CONSUMER_KEY and CONSUMER_SECRET"
            )
        }, 500
    ...
    logger.debug(
        f"STK Push - Short Code: {settings.SHORT_CODE}, "
    ...
    try:
        ...
        logger.info(
            f"STK push initiated successfully for payment "
...
def daraja_timestamp_to_datetime(timestamp_str: str) -> datetime:
    ...
    try:
        ...
        logger.error(
            f"Error parsing Daraja timestamp {timestamp_str}: "
            f"{str(e)}"
        )
        return datetime.now(timezone.utc)


def get_payment_by_checkout_request_id(
    ...
    try:
        ...
        logger.error(
            f"Error getting payment by checkout request ID: "
            f"{str(e)}",
            exc_info=True
        )
        return None
