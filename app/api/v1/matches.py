...
def _send_match_notification(match_id: UUID, partner_id: UUID) -> None:
    ...
    try:
        ...
        logger.error(f"Error sending match notification: {str(e)}", exc_info=True)
    finally:
        db.close()


def _send_chat_notification(match_id: UUID, partner_id: UUID, sender_id: UUID) -> None:
    """
    ...
    try:
        ...
        # Send notification
        notifications.send_chat_notification(match, partner.fcm_token, sender, db)
    except Exception as e:
        # Log error but don't fail - notification is non-critical
        logger.error(f"Error sending chat notification: {str(e)}", exc_info=True)
    finally:
        db.close()
...
def create_or_update_match(
    ...
    if existing_match:
        ...
        if match_data.last_message and match_data.sent_by:
            ...
            partner_id = match_data.partner_id if match_data.sent_by == match_data.my_id else match_data.my_id
