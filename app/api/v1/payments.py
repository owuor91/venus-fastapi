...
from app.schemas.payment import PaymentCreateRequest, PaymentUpdate, Payment as PaymentSchema
...
@router.post("/initiate-stk", response_model=PaymentSchema, status_code=status.HTTP_201_CREATED)
def initiate_stk_payment(
    ...
    try:
        ...
        if stk_status_code != 200:
            logger.warning(f"STK push failed for payment {db_payment.payment_id}: {stk_response}")
    ...
    try:
        ...
        for item in callback_metadata:
            ...
            if name == "Amount":
                ...
                if value:
                    from app.core.daraja_helper import daraja_timestamp_to_datetime
        ...
        if user and user.fcm_token:
            try:
                notifications.send_payment_notification(payment, user.fcm_token)
            except Exception as e:
